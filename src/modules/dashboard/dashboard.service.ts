...
export class DashboardService {
  ...
  private async getOrdersSummary(organizationId: number): Promise<OrdersSummary> {
    ...
    return {
      ...
      ordersChange: ordersChange.startsWith('-') ? ordersChange : `+${ordersChange}`
    };
  }

  private async getRevenueSummary(organizationId: number): Promise<RevenueSummary> {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
  ...
  private async getActiveConversationsSummary(organizationId: number): Promise<ActiveConversationsSummary> {
  ...
  private async getConversionRateSummary(organizationId: number): Promise<ConversionRateSummary> {
    const now = new Date();
    ...
    const [thisWeekOrders, thisWeekConversations, lastWeekOrders, lastWeekConversations] = await Promise.all([
      // This week orders
      this.prisma.order.count({
        where: {
          organizationId,
    ...
    const thisWeekRate = thisWeekConversations === 0 ? 0 : (thisWeekOrders / thisWeekConversations) * 100;
    const lastWeekRate = lastWeekConversations === 0 ? 0 : (lastWeekOrders / lastWeekConversations) * 100;

    const conversionRateChange = lastWeekRate === 0 
      ? '0%' 
      : `${((thisWeekRate - lastWeekRate) / lastWeekRate * 100).toFixed(1)}%`;

    return {
      value: Math.round(thisWeekRate * 10) / 10, // Round to 1 decimal place
      change: conversionRateChange.startsWith('-') ? conversionRateChange : `+${conversionRateChange}`
    };
  }
}
